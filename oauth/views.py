...
def emailconfirm(request, id, sign):
    ...
    if (
        not get_sha256(settings.SECRET_KEY + str(id) + settings.SECRET_KEY).upper()
        == sign.upper()
    ):
        return HttpResponseForbidden()
    ...
    content = """
     <p>恭喜您，您已经成功绑定您的邮箱，您可以使用{type}来直接免密码登录本网站.欢迎您继续关注本站，地址是</p>
                ...
                {url}
    """.format(
        type=oauthuser.type, url="http://" + site
    )
...
def bindsuccess(request, oauthid):
    ...
    return render(
        request, "oauth/bindsuccess.html", {"title": title, "content": content}
    )
