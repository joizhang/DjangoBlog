...
from blog.models import Article, Category, Links, LinkShowType, Tag
...
class ArticleDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        prev_page = (
            p_comments.previous_page_number() if p_comments.has_previous() else None
        )

        if next_page:
            kwargs["comment_next_page_url"] = (
                self.object.get_absolute_url()
                + f"?comment_page={next_page}#commentlist-container"
            )
        if prev_page:
            kwargs["comment_prev_page_url"] = (
                self.object.get_absolute_url()
                + f"?comment_page={prev_page}#commentlist-container"
            )
